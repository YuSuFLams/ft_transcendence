...
const NotFound = () => {
  ...
  const handleReturn = () => {
    ...
    if (token) {
      router.push("/dashboard"); // Navigate to the dashboard if the user is authenticated
    } else {
      router.push("/login-signup"); // Navigate to login/signup if the user is not authenticated
    }
  };
