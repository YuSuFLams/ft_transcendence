...
import { isValidInput } from '../auth/auth-utils';
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>,
    ...
    const inputValidation = isValidInput(inputRef);

    if (!inputValidation.valid) {
        setError((prev) => ({ ...prev, email: inputValidation.error, general: ''}));
        setLoading(false);
        return;
    }
    ...
    try {
        ...
        Cookie.set("email", responseData.email);
        setError({ general: '' });
    } catch (error: unknown) {
...
const handleSendCode = async (
    ...
    if (code.some(digit => !/^\d$/.test(digit))) {
        setError((prev) => ({ ...prev, enough: 'Invalid character. Only digits are allowed.', general: ''}));
        return;
    }
...
const reSendCode = async (setError: React.Dispatch<React.SetStateAction<Record<string, string>>>, resetCode: () => void) => {
    ...
    message.info({content: data || "Code sent successfully", icon: <CheckCircleOutlined style={{ color: '#52c41a', fontSize: '20px' }} />, duration: 5});
};
