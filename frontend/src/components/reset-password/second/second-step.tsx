...
import { handleContinue } from "../../api/reset-password";
...
const SecondStep: React.FC = () => {
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
        ...
        if (pastedCode.length === 6) {
            ...
            inputRefs.current.forEach((inputRef, index) => { if (inputRef) inputRef.value = pastedCode.charAt(index);});
        }
    };

    return (
        <div className="flex bg-[#011C40] rounded-b-xl relative flex-col items-center">
            {!isMe ? (
                <PreSendCode isNotMe={isNotMe} handleContinue={() => handleContinue(toggleView, resetCode, setError)} error={error} profile={profile} />
            ) : (
                <VerificationView code={code} inputRefs={inputRefs} error={error} setError={setError} resetCode={resetCode}
