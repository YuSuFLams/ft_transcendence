...
const VerificationView: React.FC<VerificationViewProps> = ({
    ...
    <div className="flex flex-col items-center justify-center mt-8 space-y-6 mb-12 md:mb-2 duration-300">
        <div className="flex flex-col space-y-4 items-center">
            ...
            <p className="text-sm font-[Font6] sm:text-base text-gray-300 m-2">Enter your Code here</p>
        </div>
        ...
        <div className="flex flex-col space-y-4 items-center">
            ...
            <motion.button className="w-[145px] sm:w-[160px] bg-[#9AB5D9] font-[Font1] text-[#142c5c] px-4 py-2 sm:py-3 rounded-lg font-semibold shadow-md 
                transition-transform text-[1em]  sm:text-[1.1em] md:text-[1.2em] ease-in-out duration-300" whileHover={{ scale: 1.05 }} disabled={loading}
                whileTap={{ scale: 0.95 }} onClick={() => handleSendCode(setError, code)}
            >
                {loading ? "Loading..." : "SEND CODE"}
            </motion.button>

            <button className="text-sm font-[Font2] sm:text-base font-medium text-gray-400 underline hover:text-white transition" onClick={() => reSendCode(setError, resetCode)}>
                Resend Code
            </button>
        </div>
    </div>
        ...
        <div className="flex flex-col items-center justify-center space-y-8 mb-4 duration-300">
            <div className="flex w-full p-2 items-center justify-between space-y-8 flex-wrap md:flex-nowrap space-x-0 md:space-x-20">
                <div className="flex items-center w-full sm:w-[50%] flex-col">
                    ...
                    <div className="flex mt-2 items-center">
                        ...
                        <span className="ml-2 text-sm sm:text-base md:text-lg text-gray-300 line-clamp-1"> {profile.email} </span>
                    </div>
                </div>
                ...
                <div className="flex flex-col items-center md:space-y-6 justify-center text-center w-full sm:w-[50%]">
                    ...
                    />
                    <p className="text-xl sm:text-2xl md:text-3xl mt-4 font-semibold text-white"> {profile.fullName !== " " ? profile.fullName : profile.username} </p>
                </div>
            </div>
            ...
            <motion.div className="flex-col space-y-4 justify-center items-center w-full">
                <motion.div className="flex justify-center lg:space-x-60 md:space-x-20 space-x-4 sm:space-x-12 items-center w-full">
                    <motion.button className="w-[100px] sm:w-[140px] font-[Font1] h-[40px] sm:h-[44px] bg-white text-black text-sm sm:text-md md:text-xl 
                        rounded-lg shadow flex items-center justify-center" whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 300 }}
                        whileTap={{ scale: 0.95 }} onClick={isNotMe}
                    >
                        It's not me
                    </motion.button>

                    <motion.button className="w-[100px] sm:w-[140px] font-[Font1] h-[40px] sm:h-[44px] bg-[#1778ff] text-white text-sm sm:text-md md:text-xl 
                        rounded-lg shadow flex items-center justify-center" whileHover={{ scale: 1.05 }} transition={{ type: "spring", stiffness: 300 }}
                        whileTap={{ scale: 0.95 }} onClick={handleContinue}
                    >
                        Continue
                    </motion.button>
                </motion.div>
