...
const HalfSideSignIn:React.FC <{toggleView: () => void}> = ({toggleView}) => {
    return (
        <div className="w-[50%] w-full h-full rounded-r-2xl flex flex-col items-center justify-center space-y-6 relative">

            <motion.div className="h-full w-full rounded-r-2xl space-y-8 flex flex-col items-center justify-center p-8 space-y-6
            ...
            >
                ...
                <div>
                    <p className="text-[#f3f3f3] text-3xl font-[TORAJA] font-extrabold text-center max-w-md">
...
const handleSubmit = async (
    ...
    if (newData.password !== newData.repassword) {
        setError((prev: any) => ({ ...prev, confirmPassword: 'Passwords do not match', general: '' }));
        return;
    }
