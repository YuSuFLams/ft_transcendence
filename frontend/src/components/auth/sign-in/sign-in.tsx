...
const SignIn: React.FC<SignInProps> = ({ toggleView, isMobile, setIsLogin }) => {
    ...
    const [inputClassName, setInputClassName] = useState<string>("w-full shadow-md shadow-[#aaabbc] p-4 pl-4 rounded-lg shadow-md \
        focus:outline-none font-[Borias] placeholder:font-[Font6] placeholder:text-xl focus:ring-2 placeholder:text-gray-500 \
    ...
    return (
        <div className="fixed flex flex-row w-full h-full items-center">
            ...
            <motion.div className={`flex-1 max-w-[500px] h-full bg-[#F3F8FF] rounded-2xl flex flex-col items-center justify-center p-4 md:p-8 space-y-4 
                lg:space-y-6 ${isMobile ? "rounded-l-2xl" : ""}`}>
                <h2 className="text-4xl md:text-6xl font-[Font3] text-black font-extrabold text-center mb-4 lg:mb-6">Sign In</h2>
                ...
                <form className="space-y-3 z-[50] md:space-y-6 w-[95%] lg:w-[90%]" onSubmit={(e) => handleSubmit(e, setError, input, data, router, setLoading, inputPassword, setIsLogin)}>
                    <div>
                        <input className={`${error.username && "border-2 border-red-500"} w-full p-4 pl-3 rounded-lg shadow-md focus:outline-none placeholder:text-lg 
                            focus:ring-2 focus:ring-[#aaabbc] placeholder:text-gray-500 focus:ring-opacity-50 font-extrabold font-[Font6] 
                            text-black text-xl md:text-2xl text-left shadow-md shadow-[#aaabbc]`} ref={input} type="text" name="username" 
                    ...
                    <div>
                        <div className="relative">
                            <input onChange={(e) => handleInputChange(e, setData, setError, inputPassword, setInputClassName)}
                                className={`${error.password ? "border-2 border-red-500" : ""} ${inputClassName}`} ref={inputPassword}
                                type={hidePass ? "password" : "text"} name="password" placeholder="Password" value={data.password}
                            />
                    ...
                    {(error.password || error.username) && <p className="text-red-600 font-[Font6] text-sm mt-1 lg:mt-1 font-bold line-clamp-2"> All fields are required</p>}
                    ...
                    <div className="flex justify-center items-center">
                        <Link className="text-black font-[TORAJA] font-extrabold text-lg md:text-xl underline" href="/reset-password">
                            Forgot password?
                        </Link>
                    </div>
                </form>
                {isMobile && <div className="z-[50] flex items-center justify-center">
                    <p className="text-[#0e213f] font-[TORAJA] text-extrabold text-lg md:text-xl">Don't have an account? <button className="text-black underline text-lg md:text-xl" onClick={toggleView}>Sign Up</button></p>
                </div>}
            </motion.div>
        </div>
    );
